...
#define LINESIZE 64
    ...
    for (j=0; j<NUM_RUNS; j++) {
        if(j > 10) {
          hbsc_acc_pow(&heart, j, 1, 0);
          if (j != 0 && j % hb_acc_pow_get_window_size(hbsc_acc_pow_get_hb(&heart)) == 0) {
            double powercap = copper_adapt(cop, j, hb_acc_pow_get_window_perf(hbsc_acc_pow_get_hb(&heart)));
            if (powercap <= 0) {
                perror("copper_adapt");
            } else {
